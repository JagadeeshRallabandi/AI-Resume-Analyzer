# Developed by dnoobnerd [https://jagadeeshrportfolio.vercel.app/]    Made with Streamlit
...
# sql connector
# connection = pymysql.connect(host='localhost',user='resume_user',password='Strong_Password_123!',db='cv')
connection = pymysql.connect(
    host='localhost',
    user='resume_user',             # the username you just created
    password='Strong_Password_123!',# the password you just set
    db='cv'
)
...
def run():
    ...
    if choice == 'User':
        ...
        if latlong:  # only try reverse lookup if lat/long found
            location = geolocator.reverse(latlong, language='en')
            if location and location.raw.get('address'):
                ...
                st.warning("Could not retrieve location details.")
        else:
            st.warning("Could not detect your coordinates.")
    ...
    ###### CODE FOR ADMIN SIDE (ADMIN) ######
        ...
        if st.button('Login'):
            ...
            if ad_user == 'admin' and ad_password == 'admin@password':
                ...
                st.success("Welcome Jagadeesh ! Total %d " % values + " User's Have Used Our Tool : )")                
